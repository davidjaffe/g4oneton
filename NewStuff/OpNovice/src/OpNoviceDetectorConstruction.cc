 ...
 : G4VUserDetectorConstruction()
{
  ...
  fBubble_x  = fBubble_y  = fBubble_z  =  0.5*cm;
  fcyl_innerrad = 0*cm;
  fcyl_outerrad = 2.5*cm;
  fcyl_z        = 3.0*cm;
  fcyl_phimin   = 0.*deg;
  fcyl_phimax   = 360.*deg;
  fvial_thickness = 0.1*cm;
  fsheet_z      = 0.25*cm;
  fsheet_x = fsheet_y = 2.5*cm;
}
...
//
  ...
  G4Element* C = new G4Element("Carbon","C",z=12,a=12.0*g/mole);
...
// pyrex from Daya Bay
//
  G4double energy_abslen_glass[] = {  1.55*eV, 
			       1.61*eV, 
			       2.07*eV,
			       2.48*eV,
			       3.56*eV,
			       ...
			       10.33*eV,
			       15.5*eV };
  G4double abslen_glass[] = {  0.1e-3*mm,
				0.5e3*mm,
				1.0e3*mm,
				2.0e3*mm,
				1.0e3*mm, 
...
/*
  ...
  G4VPhysicalVolume* waterTank_phys
    = new G4PVPlacement(0,G4ThreeVector(),waterTank_log,"Tank",
                        expHall_log,false,0);
*/
// glass vial: for now, just a cylinder of pyrex with no cap
  G4Tubs* vial_cyl = new G4Tubs("cylVial", fcyl_innerrad, fcyl_outerrad+fvial_thickness, fcyl_z+fvial_thickness, fcyl_phimin, fcyl_phimax );
  G4LogicalVolume* vial_log = new G4LogicalVolume(vial_cyl, pyrex,"cylvial",0,0,0);
  G4VPhysicalVolume* vial_phys = new G4PVPlacement(0,G4ThreeVector(),vial_log,"cylvial",expHall_log,false,0);

// water cylinder inside glass vial
  G4Tubs* waterTank_cyl = new G4Tubs("cylTank",  fcyl_innerrad, fcyl_outerrad, fcyl_z, fcyl_phimin, fcyl_phimax );
  G4LogicalVolume* waterTank_log = new G4LogicalVolume(waterTank_cyl,water,"cylTank",0,0,0);
  G4VPhysicalVolume* waterTank_phys = new G4PVPlacement(0,G4ThreeVector(),waterTank_log,"cylTank",vial_log,false,0);

  // acrylic sheet
  G4Box* acrylicSheet = new G4Box("AcrylicSheet",fsheet_x,fsheet_y,fsheet_z);
  G4LogicalVolume* acrylicSheet_log = new G4LogicalVolume(acrylicSheet,Acrylic,"acrSheet",0,0,0);
  G4VPhysicalVolume* acrylicSheet_phys = new G4PVPlacement(0,G4ThreeVector(0,0,-(fcyl_z+1.*cm)),acrylicSheet_log,"acrSheet",expHall_log,false,0);
...
//
  ...
  new G4LogicalBorderSurface("WaterSurface",
                                 waterTank_phys,vial_phys,opWaterSurface);
